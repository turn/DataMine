...
public class RunLengthEncodingTest {
	...
	IEncoding encoder = new RunLengthEncoding();
	...
	public void estimateSize() {
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				new CompressStats(values1, type)), 5 * 4);
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				...
				1 + 2 + "1111".length() + 1 + 2 + "aaaaa".length());
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				new CompressStats(values1, type)), 4 * (1+2+"test".length()));
