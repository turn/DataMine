...
public class CommonValueEncodingTest {
	
	IEncoding encoder = new CommonValueEncoding();
	
	@Test 
	public void trail() {
		Map<Object, Integer> m = Maps.newHashMap();
		m.put(null, 0);
		m.put("ertar", 1);
	...
	public void estimateSize() {
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				new CompressStats(values1, type)), 5);
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				new CompressStats(values1, type)), 4 + (1000 + 7)/8);
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				...
				1 + 2 + "1111".length() + 2 + "aaaaa".length());
		...
		Assert.assertEquals(encoder.getEstimatedSize(values1, type, 
				new CompressStats(values1, type)), (1000 + 7)/8 + (2+"test".length()));
